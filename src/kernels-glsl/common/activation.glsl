...
#else
	// not every compute shader will have the same constant id for activation
	#define PREPARE_ACTIVATION(cid) layout(constant_id = cid) uint ACTIVATION = ACTIVATION_IDENTITY;
