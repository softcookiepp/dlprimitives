...
namespace core {
    ...
    public:
...
#if VULKAN_API
				sums_->run({second_reduce_,features_}, {});
				e.queue()->sync();
				sums_reduce_->run({1, features_}, {});
#else
                e.queue().enqueueNDRangeKernel( sums_,
                                                cl::NullRange,
...
#endif
#if VULKAN_API
			update_sums_->run({features_}, {1, 1, 1});
#else
                                            ...
                                            Tensor &ws,ExecutionContext const &e)
        {
            ...
            auto e2=e.generate_series_context(1,2);
#if VULKAN_API
			mean_var_to_a_b_->run({features_}, {1, 1, 1});
#else
                                            ...
                                            Tensor &ws,ExecutionContext const &e)
        {
            ...
            auto e2=e.generate_series_context(1,2);
#if VULKAN_API
			combine_mean_var_with_gamma_beta_->run({features_}, {1, 1, 1});
#else
...
#if VULKAN_API
			compute_backward_factors_->run({features_}, {1, 1, 1});
#else
...
#endif
#if VULKAN_API
			backward_filter_->run({features_}, {1, 1, 1});
#else
