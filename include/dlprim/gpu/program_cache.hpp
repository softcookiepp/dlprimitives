...
namespace dlprim {
    namespace gpu {
        ...
        public:
			...
			static tart::program_ptr build_program(const tart::device_ptr& device, std::string const &source)
            {
                std::vector<Parameter> p;
				return build_program(device, source,p);
            }
			static tart::program_ptr build_program(const tart::device_ptr& device, std::string const &source,std::vector<Parameter> const &params);
        private:
